...
extension RecipePresenter {
    ...
    private static func makeDetailsHeaderViewProps(state: State) -> RecipeDetailsHeaderView.Props {
        return .init(
            ...
            ratingViewProps: RatingView.makeProps(recipeDetails: state.recipeDetails.getModel()),
            timeViewProps: RecipeTimeView.Props(
                timeDescription: .recipeDetailsCookingTime(state.recipeDetails.duration)
            )
        )
    }
